...
- `components/tools/ppt/SlideGenerationResult.tsx` — "Open in Google Slides" link + missing-hymn warnings + the full program the deck was built from, via `WorshipOrderSummary.tsx`. `page.tsx` keeps the verbatim step-1 paste (`rawText`, the second arg of `WorshipTextInput`'s `onParsed`) and, on success, adopts the review form's edited copy (`setParsed(data)`) so the summary shows what was actually generated, not the pre-edit parse.
- `components/tools/ppt/WorshipOrderSummary.tsx` — read-only render of a `WorshipOrderData`: service date / sermon title+subtitle / speaker / 圣餐, hymns with 回应诗歌 and YouTube badges, the five scripture refs labelled from `SCRIPTURE_FIELDS` (宣召 replaced by its custom text when set, empty refs skipped), `otherLines`, and the raw paste in a `<details>`. Shared by step 3 and the generated-decks list; tolerates a `data` blob straight out of the Json column.
- `app/[locale]/admin/ppt/generated/page.tsx` — separate read-only page listing decks already in the output folder (sidebar key `generatedPpt`). A deck with a stored program renders its title as a toggle button (chevron, `aria-expanded`/`aria-controls`) that expands `WorshipOrderSummary` inline; decks generated before programs were persisted keep a plain title with no affordance.
...
7. Saves the program the deck was built from: one `WorshipOrder` row with `presentationId`/`presentationUrl`, `date` = the service Sunday, `name` = the compact-date title, `rawText` = the operator's paste, and `data` = the `WorshipOrderData` snapshot (`items` stays empty — these rows are snapshots, not the item-based rows the `worship-orders` CRUD routes create). Best-effort: the deck already exists, so a DB failure is logged and the request still succeeds.
8. Audit-logs (`resourceType: "GoogleSlides"`) and returns `{ presentationUrl, presentationId, missingHymns }`.
...
**Listing generated decks** (`GET /api/admin/ppt/generated`): `lib/google-drive.ts` `listGeneratedPresentations(folderId)` lists presentations in `GOOGLE_SLIDES_OUTPUT_FOLDER_ID` via the Drive API — **Drive is still the source of truth for which decks exist.** Each file is then joined against `WorshipOrder.presentationId` (one `findMany`) and returned as `{ ...file, program: { serviceDate, rawText, data } | null }`; `program` is null for decks with no stored snapshot.
